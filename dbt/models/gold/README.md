...
redesenhar depois de a bronze ficar 10x maior. `dim_perfil_paciente` foi
adicionada em 23/08/2026 (adendo ao ADR 0008), já com os 7 meses carregados.

- `fct_cobertura_vacinal` — fato, grão **agregado**: uma linha por
  combinação de (`data_vacina`, `chave_municipio`, `chave_vacina`,
  `chave_faixa_etaria`, `chave_perfil`), métrica `qtd_doses`. Não é uma
  linha por dose — ver ADR 0008 pro porquê (tamanho/velocidade no Power BI
  + reforço do ADR 0005/LGPD).
...
Validado com `dbt build` contra fevereiro/2026 real (PASS=31); ainda não
revalidado contra os 7 meses completos — próximo passo.
